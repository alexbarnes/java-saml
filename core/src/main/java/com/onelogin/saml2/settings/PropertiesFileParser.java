...
/**
 * Parser for {@linkplain Saml2Settings} defined in a {@linkplain Properties} file.
 *
 */
public class PropertiesFileParser {
   ...
   * @param prop
   */
  public static void parse(Saml2Settings saml2Setting, Properties prop) {
    Boolean strict = loadBooleanProperty(STRICT_PROPERTY_KEY, prop);
    if (strict != null)
      saml2Setting.setStrict(strict);

    Boolean debug = loadBooleanProperty(DEBUG_PROPERTY_KEY, prop);
    if (debug != null)
      saml2Setting.setDebug(debug);

    loadSpSetting(saml2Setting, prop);
    loadIdpSetting(saml2Setting, prop);
    loadSecuritySetting(saml2Setting, prop);
    loadCompressSetting(saml2Setting, prop);

    saml2Setting.setContacts(loadContacts(prop));
    saml2Setting.setOrganization(loadOrganization(prop));
   ...
   */
  private static void loadIdpSetting(Saml2Settings saml2Setting, Properties prop) {
    String idpEntityID = loadStringProperty(IDP_ENTITYID_PROPERTY_KEY, prop);
    if (idpEntityID != null)
      saml2Setting.setIdpEntityId(idpEntityID);

    URL idpSingleSignOnServiceUrl = loadURLProperty(IDP_SINGLE_SIGN_ON_SERVICE_URL_PROPERTY_KEY, prop);
    if (idpSingleSignOnServiceUrl != null)
      saml2Setting.setIdpSingleSignOnServiceUrl(idpSingleSignOnServiceUrl);

    String idpSingleSignOnServiceBinding = loadStringProperty(IDP_SINGLE_SIGN_ON_SERVICE_BINDING_PROPERTY_KEY, prop);
    if (idpSingleSignOnServiceBinding != null)
      saml2Setting.setIdpSingleSignOnServiceBinding(idpSingleSignOnServiceBinding);

    URL idpSingleLogoutServiceUrl = loadURLProperty(IDP_SINGLE_LOGOUT_SERVICE_URL_PROPERTY_KEY, prop);
    if (idpSingleLogoutServiceUrl != null)
      saml2Setting.setIdpSingleLogoutServiceUrl(idpSingleLogoutServiceUrl);

    URL idpSingleLogoutServiceResponseUrl = loadURLProperty(IDP_SINGLE_LOGOUT_SERVICE_RESPONSE_URL_PROPERTY_KEY, prop);
    ...
    String idpSingleLogoutServiceBinding = loadStringProperty(IDP_SINGLE_LOGOUT_SERVICE_BINDING_PROPERTY_KEY, prop);
    if (idpSingleLogoutServiceBinding != null)
      saml2Setting.setIdpSingleLogoutServiceBinding(idpSingleLogoutServiceBinding);

    X509Certificate idpX509cert = loadCertificateFromProp(IDP_X509CERT_PROPERTY_KEY, prop);
    if (idpX509cert != null)
      saml2Setting.setIdpx509cert(idpX509cert);

    String idpCertFingerprint = loadStringProperty(CERTFINGERPRINT_PROPERTY_KEY, prop);
    if (idpCertFingerprint != null)
      saml2Setting.setIdpCertFingerprint(idpCertFingerprint);

    String idpCertFingerprintAlgorithm = loadStringProperty(CERTFINGERPRINT_ALGORITHM_PROPERTY_KEY, prop);
   ...
   */
  private static void loadSecuritySetting(Saml2Settings saml2Setting, Properties prop) {
    Boolean nameIdEncrypted = loadBooleanProperty(SECURITY_NAMEID_ENCRYPTED, prop);
    if (nameIdEncrypted != null)
      saml2Setting.setNameIdEncrypted(nameIdEncrypted);

    Boolean authnRequestsSigned = loadBooleanProperty(SECURITY_AUTHREQUEST_SIGNED, prop);
    if (authnRequestsSigned != null)
      saml2Setting.setAuthnRequestsSigned(authnRequestsSigned);

    Boolean logoutRequestSigned = loadBooleanProperty(SECURITY_LOGOUTREQUEST_SIGNED, prop);
    if (logoutRequestSigned != null)
      saml2Setting.setLogoutRequestSigned(logoutRequestSigned);

    Boolean logoutResponseSigned = loadBooleanProperty(SECURITY_LOGOUTRESPONSE_SIGNED, prop);
    if (logoutResponseSigned != null)
      saml2Setting.setLogoutResponseSigned(logoutResponseSigned);

    Boolean wantMessagesSigned = loadBooleanProperty(SECURITY_WANT_MESSAGES_SIGNED, prop);
    if (wantMessagesSigned != null)
      saml2Setting.setWantMessagesSigned(wantMessagesSigned);

    Boolean wantAssertionsSigned = loadBooleanProperty(SECURITY_WANT_ASSERTIONS_SIGNED, prop);
    if (wantAssertionsSigned != null)
      saml2Setting.setWantAssertionsSigned(wantAssertionsSigned);

    Boolean wantAssertionsEncrypted = loadBooleanProperty(SECURITY_WANT_ASSERTIONS_ENCRYPTED, prop);
    if (wantAssertionsEncrypted != null)
      saml2Setting.setWantAssertionsEncrypted(wantAssertionsEncrypted);

    Boolean wantNameId = loadBooleanProperty(SECURITY_WANT_NAMEID, prop);
    if (wantNameId != null)
      saml2Setting.setWantNameId(wantNameId);

    Boolean wantNameIdEncrypted = loadBooleanProperty(SECURITY_WANT_NAMEID_ENCRYPTED, prop);
    if (wantNameIdEncrypted != null)
      saml2Setting.setWantNameIdEncrypted(wantNameIdEncrypted);

    Boolean wantXMLValidation = loadBooleanProperty(SECURITY_WANT_XML_VALIDATION, prop);
    if (wantXMLValidation != null)
      saml2Setting.setWantXMLValidation(wantXMLValidation);

    Boolean signMetadata = loadBooleanProperty(SECURITY_SIGN_METADATA, prop);
    if (signMetadata != null)
      saml2Setting.setSignMetadata(signMetadata);

    List<String> requestedAuthnContext = loadListProperty(SECURITY_REQUESTED_AUTHNCONTEXT, prop);
    if (requestedAuthnContext != null)
      saml2Setting.setRequestedAuthnContext(requestedAuthnContext);

    String requestedAuthnContextComparison = loadStringProperty(SECURITY_REQUESTED_AUTHNCONTEXTCOMPARISON, prop);
    ...
    String signatureAlgorithm = loadStringProperty(SECURITY_SIGNATURE_ALGORITHM, prop);
    if (signatureAlgorithm != null && !signatureAlgorithm.isEmpty())
      saml2Setting.setSignatureAlgorithm(signatureAlgorithm);

    Boolean rejectUnsolicitedResponsesWithInResponseTo = loadBooleanProperty(SECURITY_REJECT_UNSOLICITED_RESPONSES_WITH_INRESPONSETO, prop);
   ...
   */
  private static void loadSpSetting(Saml2Settings saml2Setting, Properties prop) {
    String spEntityID = loadStringProperty(SP_ENTITYID_PROPERTY_KEY, prop);
    if (spEntityID != null)
      saml2Setting.setSpEntityId(spEntityID);

    URL assertionConsumerServiceUrl = loadURLProperty(SP_ASSERTION_CONSUMER_SERVICE_URL_PROPERTY_KEY, prop);
    if (assertionConsumerServiceUrl != null)
      saml2Setting.setSpAssertionConsumerServiceUrl(assertionConsumerServiceUrl);

    String spAssertionConsumerServiceBinding = loadStringProperty(SP_ASSERTION_CONSUMER_SERVICE_BINDING_PROPERTY_KEY, prop);
    ...
    URL spSingleLogoutServiceUrl = loadURLProperty(SP_SINGLE_LOGOUT_SERVICE_URL_PROPERTY_KEY, prop);
    if (spSingleLogoutServiceUrl != null)
      saml2Setting.setSpSingleLogoutServiceUrl(spSingleLogoutServiceUrl);

    String spSingleLogoutServiceBinding = loadStringProperty(SP_SINGLE_LOGOUT_SERVICE_BINDING_PROPERTY_KEY, prop);
    if (spSingleLogoutServiceBinding != null)
      saml2Setting.setSpSingleLogoutServiceBinding(spSingleLogoutServiceBinding);

    String spNameIDFormat = loadStringProperty(SP_NAMEIDFORMAT_PROPERTY_KEY, prop);
    if (spNameIDFormat != null && !spNameIDFormat.isEmpty())
      saml2Setting.setSpNameIDFormat(spNameIDFormat);

    X509Certificate spX509cert = loadCertificateFromProp(SP_X509CERT_PROPERTY_KEY, prop);
    if (spX509cert != null)
      saml2Setting.setSpX509cert(spX509cert);

    PrivateKey spPrivateKey = loadPrivateKeyFromProp(SP_PRIVATEKEY_PROPERTY_KEY, prop);
   ...
   */
  private static void loadCompressSetting(Saml2Settings saml2Setting, Properties prop) {
    Boolean compressRequest = loadBooleanProperty(COMPRESS_REQUEST, prop);
    if (compressRequest != null) {
      saml2Setting.setCompressRequest(compressRequest);
    }

    Boolean compressResponse = loadBooleanProperty(COMPRESS_RESPONSE, prop);
   ...
   */
  private static Organization loadOrganization(Properties prop) {
    Organization orgResult = null;

    String orgName = loadStringProperty(ORGANIZATION_NAME, prop);
    String orgDisplayName = loadStringProperty(ORGANIZATION_DISPLAYNAME, prop);
    URL orgUrl = loadURLProperty(ORGANIZATION_URL, prop);
    String orgLangAttribute = loadStringProperty(ORGANIZATION_LANG, prop);
   ...
   */
  private static List<Contact> loadContacts(Properties prop) {
    List<Contact> contacts = new LinkedList<>();

    String technicalGn = loadStringProperty(CONTACT_TECHNICAL_GIVEN_NAME, prop);
    String technicalEmailAddress = loadStringProperty(CONTACT_TECHNICAL_EMAIL_ADDRESS, prop);
    ...
    String supportGn = loadStringProperty(CONTACT_SUPPORT_GIVEN_NAME, prop);
    String supportEmailAddress = loadStringProperty(CONTACT_SUPPORT_EMAIL_ADDRESS, prop);
   ...
   * @return the value
   */
  private static String loadStringProperty(String propertyKey, Properties prop) {
   ...
   * @return the value
   */
  private static Boolean loadBooleanProperty(String propertyKey, Properties prop) {
   ...
   * @return the value
   */
  private static List<String> loadListProperty(String propertyKey, Properties prop) {
   ...
   * @return the value
   */
  private static URL loadURLProperty(String propertyKey, Properties prop) {
    String urlPropValue = prop.getProperty(propertyKey);
    return Util.createUrl(urlPropValue);
  }
   ...
   */
  private static X509Certificate loadCertificateFromProp(String propertyKey, Properties prop) {
   ...
   */
  protected static PrivateKey loadPrivateKeyFromProp(String propertyKey, Properties prop) {
